                ...
                <div className='relative'>
                    ...
                    <div className={`absolute bg-gray-700 rounded-b-lg transition-all ring-2 ring-white group-hover:ring-gray-900 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                        {options.map(({ key, value }, i) => (
                            <button className=' text-white text-xs block w-full justify-center px-2 py-1 hover:bg-gray-600' type="button" value={value} onClick={() => this.handleClick(value, i)}>{key}</button>
                        ))}
                    </div>
                </div>
            </div>
