...
const SelectInput = ({ placeholder = '', value = "", onChange, className = "", prefix, suffix, options = [] }) => {
    ...
    return (
        <InputWrapper {...{ className, prefix, suffix }}>
            <select disabled={!(onChange && (typeof onChange === "function"))} tabIndex="0" style={{ fontWeight: 'inherit' }} className={`p-1  w-full block bg-transparent outline-none appearance-none ${value ? '' : 'text-gray-400 print:hidden'}`} {...{ value, placeholder }} onChange={e => onChange(e.currentTarget.value)}>
